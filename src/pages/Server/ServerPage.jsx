...
const ServerPage = () => {
  ...
  const fetchChapterContent = async (link_manga) => {
    const res = await axios.get(link_manga);
  ...
  // const fetchChapterDetailNovel = async () => {
  //   try {
  ...
  const fetchChapterDetail = async () => {
    try {
      const response = await axios.get(`https://apimanga.mangasocial.online/rmanga/${slug}`);
      setChapterDetail(response.data);
      setCurrentChap(response.data[0].chapters[0]);
      setListChap(response.data[0].chapters);
      console.log("chapter detail:", response.data);
